...
#metodo para traer los datos
    def get_graph_sales_year_month(self):
        data = []
        try:
            year = datetime.now().year #anio actual
            for mes in range(1, 13):
                # total = Sale.objects.filter(date_joined__year=year, date_joined__month=mes).aggregate(r=Coalesce(Sum('total'), 0)).get('r')
                # data.append(float(total)) #sumatoria en tipo arreglo
                total = Sale.objects.filter(date_joined__year=year, date_joined__month=mes).aggregate(
                    r=Coalesce(Sum('total'), 0)).get('r') #sumatoria tipo Jrson
                data.append(float(total))
        except:
            pass
        return data

#metodo para grafico de ventas de prodyctos al anio
    ...
    def get_context_data(self, **kwargs):
        ...
        context['graph_sales_year_month'] = self.get_graph_sales_year_month() #datos por medio del metodo get_graph
        return context